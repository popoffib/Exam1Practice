...
def problem2b(rect, n, delta, win):
    ...
    rect.attach_to(win)
    for k in range(n - 1):
        l_corner_x = rect.get_lower_left_corner().x - delta
        l_corner_y = rect.get_lower_left_corner().y + delta
        r_corner_x = rect.get_upper_right_corner().x + delta
        r_corner_y = rect.get_upper_right_corner().y - delta
        rect = rg.Rectangle(rg.Point(l_corner_x, l_corner_y), rg.Point(r_corner_x, r_corner_y))
        rect.attach_to(win)
        win.render()
